...
const initialGoals: Goal[] = [
  {
    ...
    description: "Transition to completely organic growing methods with natural pesticides and fertilizers",
    type: "long-term",
    progress: 45,
    completed: false,
  },
  {
    ...
    description: "Add compost and natural amendments to increase nutrient content by 30%",
    type: "long-term",
    progress: 60,
    completed: false,
  },
...
export function GoalsProvider({ children }: { children: ReactNode }) {
  ...
  const updateGoalDueDate = (id: string, date: string | undefined) => {
    setGoals((prev) =>
      prev.map((g) => (g.id === id ? { ...g, dueDate: date } : g))
    );
  };

  return (
    <GoalsContext.Provider value={{ goals, setGoals, toggleComplete, updateGoalDueDate }}>
      {children}
